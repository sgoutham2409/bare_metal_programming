...
extern uint32_t __start_of_bss;
extern uint32_t __end_of_bss;
...
void Reset_Handler(void)
{
	...
	uint32_t size = (uint32_t)&__end_of_data - (uint32_t)&__start_of_data;

	uint8_t *pDst = (uint8_t*)&__start_of_data; // SRAM
	uint8_t *pSrc = (uint8_t*)&__end_of_text;   // FLASH
	...
	size = (uint32_t)&__end_of_bss - (uint32_t)&__start_of_bss;
	pDst = (uint8_t*)&__start_of_bss;
	for (uint32_t i = 0; i < size; i++)
	{
		*pDst++ = 0;
	}
	...
	// call init function of std library : This is required if there is a dependency on any std lib function.
	// For ex, printf, string functions, etc,.
